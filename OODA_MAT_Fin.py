...
This prototype:
- runs locally without an external LLM API;
...
TOOL_LINKS = {
    "Streamlit": {
        ...
        "example": "pip install streamlit; streamlit run OODA_MAT_Fin.py",
...
with st.sidebar:
    ...
    if uploaded is not None:
        try:
            ...
            incoming, missing = ensure_columns(incoming)
            if missing:
                st.error("Missing columns: " + ", ".join(missing))
            else:
                st.session_state.data = incoming
                st.success(f"Loaded {len(incoming)} rows")
...
tabs = st.tabs([
    ...
    "4 Act", "5 Feedback", "Tools", "Conversation", "Hydration Simulation"
])
...
with tabs[1]:
    ...
    st.download_button("Download validated dataset", csv, "OODA_MAT_validated_data.csv", "text/csv")
...
with tabs[2]:
    ...
    st.dataframe(engineered_features(df).head(20), width="stretch")
    ...
    st.dataframe(hydration_model_catalog(models), width="stretch", hide_index=True)
...
with tabs[4]:
    ...
    if cand is None or cand.empty:
        ...
        st.dataframe(pd.DataFrame(mass_rows), width="stretch", hide_index=True)
...
with tabs[5]:
    ...
    if st.session_state.feedback_log:
        ...
        st.download_button(
            ...
            fb.to_csv(index=False).encode("utf-8-sig"),
...
with tabs[7]:
    ...
    prompt = st.chat_input("Ask about Observe, Orient, Decide, Act, Feedback or tools")
    if prompt:
    ...
    if embedded_run:
        ...
        structure_3d.add_trace(go.Scatter3d(
            ...
            hovertemplate=(
                ...
                "<br>Reaction degree=%{customdata[1]:.3f}<extra></extra>"
            ),
        ))
        structure_3d.add_trace(go.Scatter3d(
            x=particle_cloud["x"], y=particle_cloud["y"], z=particle_cloud["z"],
...
st.divider()
st.caption(
    "OODA-MAT Final v3.2. Demonstration data are synthetic. "
